 ...
 *
 * Extracted from mcp-server.js so both the MCP server and the
 * chat REPL can call the same handlers with their own session instances.
 *
 * Usage:
 *   const { TOOLS, createToolHandlers } = await import('./tools/handlers.js');
 *   const handlers = createToolHandlers({ memory, diffEngine, budget });
...
// ============================================================
//  Tool Definitions (shared between MCP + Chat)
// ============================================================
...
/**
 * Create tool handlers bound to the given session instances.
 * @param {{ memory: SessionMemory, diffEngine: DiffEngine, budget: TokenBudget }} deps
 * @returns {Object<string, function>} Map of tool name → async handler
 */
export function createToolHandlers({ memory, diffEngine, budget }) {

  // ── Capture Handlers ──

  async function handleGetScreenState() {
    const state = await captureState();
    if (!state) return "Error: Could not capture screen state. Check Accessibility permissions.";
    memory.record(state);
  ...
  async function handleGetAmbient() {
    ...
    if (!state) return "Error: Could not capture ambient state.";
  ...
  async function handleGetScreenshotPath(args) {
    ...
    if (!path) return "Error: Screenshot failed.";
    return `Screenshot saved to: ${path}`;
  }

  async function handleEstimateTokens() {
    const state = await captureState();
    if (!state) return "Error: Could not capture state.";
  ...
  async function handleSetBudget(args) {
    const { TokenBudget } = await import('../budget/token-budget.js');
    const newBudget = new TokenBudget({ budget: args.preset, model: budget.model });
